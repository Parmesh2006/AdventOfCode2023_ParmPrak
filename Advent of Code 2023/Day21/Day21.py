#!/usr/bin/python

"""
THIS IS THE CORRECT SOLUTION.

AUTHOR: Parmeshvar Prakash
"""

# Dictionary identifying coordinates for each element in the input
G = {i + j * 1j: c for i, r in enumerate(open('Day21\\input.txt'))
              for j, c in enumerate(r) if c in '.S'}

# Keeps track of coordinates that were passed over and the ones to do
done = []
todo = {x for x in G if G[x] == 'S'}
cmod = lambda x: complex(x.real % 131, x.imag % 131)

# Iterates until the possibilities are filled
for s in range(393):
    if s == 64: 
        print(len(todo))
    if s % 131 == 65: 
        done.append(len(todo))

    todo = {p + d for d in {1, -1, 1j, -1j}
                for p in todo if cmod(p + d) in G}

# Part One
f = lambda n, a, b, c: a + n * (b - a + (n - 1) * (c - b - b + a) // 2)

# Part Two
print(f(26501365 // 131, *done))